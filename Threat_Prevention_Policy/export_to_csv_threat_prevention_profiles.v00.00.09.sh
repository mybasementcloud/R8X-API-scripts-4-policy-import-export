...
echo 'Generate Threat Prevention Profiles detailed export for reference' | tee -a -i ${logfilepath}
echo | tee -a -i ${logfilepath}

#export exportfileheader=${exportfileprefix}.${forreferenceonlytext}.header.${exportfileext}

export exportfile=${exportfilepath4reference}/${exportfileprefix}.${forreferenceonlytext}.${localnamenow}.${exportfileext}
...
cat ${showfile} | ${JQ} -r '.profiles[] | [ '"${jsonvaluekeys}"' ] | @csv ' >> ${exportfile} 
...
# -------------------------------------------------------------------------------------------------
# Generate Threat Prevention Profiles detailed export for actual import with extended attributes - MAY NOT WORK!
# -------------------------------------------------------------------------------------------------
...
#export export2exportfileheader=${exportfilepath}/${exportfileprefix}.export.with_IPS_ext_attributes.header.${exportfileext}

export export2exportfile=${exportfilepath}/${exportfileprefix}.export.with_IPS_ext_attributes.${localnamenow}.${exportfileext}
...
echo ${csvheader} > ${export2exportfile} 

cat ${showfile} | ${JQ} -r '.profiles[] | [ '"${jsonvaluekeys}"' ] | @csv ' >> ${export2exportfile} 
